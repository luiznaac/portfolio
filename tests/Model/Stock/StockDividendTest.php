...
class StockDividendTest extends TestCase {
    ...
    public function testGetStockDividendsForDateRangeWithoutStoredDividendsWithFII_ShouldLoadAndReturnValues(): void {
        ...
        $stock_dividends = StockDividend::getStockDividendsForDateRange($stock,  Carbon::today()->subMonths(6), Carbon::today());

        $this->assertStockDividends($expected_data, $stock_dividends);
    }

    public function testGetStockDividendsForDateRangeWithoutStoredDividendsWithAcao_ShouldLoadAndReturnValues(): void {
        $stock = new Stock();
        $stock->symbol = 'FLRY3';
        $stock->save();
